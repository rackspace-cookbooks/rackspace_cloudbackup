...
describe 'RcbuCache' do
  before :each do
    # Require ChefSpec within the test scope to avoid colissions with WebMock
    require 'chefspec_helper'
    require_relative '../../../libraries/RcbuCache.rb'

    # Test a 3 level cache, which should be quite sufficient to exercise the code.
    @test_cache = Opscode::Rackspace::CloudBackup::RcbuCache.new(3)

    # Create a second cache to test for crosstalk due to some unforseen insanity
    # (We *are* allowing class variables, after all)
    @second_cache = Opscode::Rackspace::CloudBackup::RcbuCache.new(3)
  end

  describe '#new' do
    it 'takes a parameter and is a RcbuCache object' do
      @test_cache.should be_an_instance_of Opscode::Rackspace::CloudBackup::RcbuCache
    end
  end
